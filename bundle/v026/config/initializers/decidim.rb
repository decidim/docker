...
Decidim.configure do |config|
  ...
  config.available_locales = ENV.fetch("DECIDIM_AVAILABLE_LOCALES", "en").split(",").map {|l| l.strip.to_sym} if ENV["DECIDIM_AVAILABLE_LOCALES"].present?
  ...
  config.force_ssl = ["enabled", "true", "1"].include?(ENV.fetch("RAILS_FORCE_SSL", "false"))
